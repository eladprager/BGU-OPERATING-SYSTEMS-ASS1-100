...
found:
  ...
  for(p2 = ptable.proc; p2 < &ptable.proc[NPROC]; p2++){
    if ((p2->accumulator<p_min->accumulator) && (p2->state == RUNNABLE || p2->state == RUNNING))
      p_min = p2;
...
proc_info(struct perf* performance)
{
    ...
    // cprintf("%d\t%d\t\t%d\t%d\t%d\n",myproc()->pid,performance->ps_priority,performance->stime,performance->retime,performance->rtime);
    return 0;
}
  ...
  for(;;){
    ...
    case 1: 
      ...
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        ...
        struct proc *p2=p;   
        for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
          if ((p->accumulator<p2->accumulator) && (p->state == RUNNABLE))
            p2 = p;
        }
        c->proc = p2;
        switchuvm(p2);
    ...
    case 2: 
      ...
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        ...
        struct proc *p2=p;
        for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
            if ((p->state == RUNNABLE) && ((double)(((p->rtime)*(p->decay_factor))/(p->stime+p->rtime+p->retime)) < (double)(((p2->rtime)*(p2->decay_factor))/(p2->stime+p2->rtime+p2->retime))))
              p2 = p;
        }
        c->proc = p2;
        switchuvm(p2);
...
yield(void)
{
  ...
  int priority = myproc()->ps_priority;
  myproc()->accumulator+=priority;
